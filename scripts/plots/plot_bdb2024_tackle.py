#!/usr/bin/env python3
"""Plot checksum-validated BDB2024 tackle full100 learning curves."""
...
DEFAULT_OUTPUT_DIR = ROOT / "results" / "bdb" / "full100_serial_bdb2024_attempt1" / "plots"


def load_primary_metrics(path: str | Path) -> pd.DataFrame:
    """Validate the exact checksum-bound 3,000-cell BDB2024 primary grid."""
...
def plot_learning_curves(data: pd.DataFrame, output_stem: Path):
    ...
    fig.suptitle(
        "BDB2024 tackle probability confirmatory learning curves",
        x=0.5,
        y=0.978,
        fontsize=13.0,
        fontweight="semibold",
    ...
    return BASE._save_figure(
        fig,
        output_stem,
        "BDB2024 tackle 100-repeat confirmatory learning curves",
    )
...
def plot_all(input_path: str | Path, output_dir: str | Path):
    ...
    return plot_learning_curves(
        data,
        destination / "bdb2024_tackle_full100_learning_curves",
    )
